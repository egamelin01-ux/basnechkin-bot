...
{{
    ...
    def process_story_request(
        ...
        try:
            ...
            if request_type == "new_dilemma":
                ...
                # Пожелания - генерируем сказку с учетом пожеланий пользователя
                prompt = f"Напиши сказку с учетом следующих пожеланий пользователя: {user_message}. " \
                        f"Сказка должна быть интересной и поучительной, но мораль НЕ должна быть написана текстом - она должна быть понятна из действий и выбора героя."
    ...
    def generate_reflection_questions(
        ...
        try:
            ...
            system_prompt = f"""Ты — помощник для бота "Сказочник", который помогает родителям развивать рефлексию у детей через вопросы после прочтения сказки.

Твоя задача:
Сгенерировать 3 вопроса для размышлений на основе прочитанной сказки.
...
ВАЖНО: Возвращай ТОЛЬКО валидный JSON без пояснений и комментариев."""
            ...
            user_prompt = f"""Прочитай следующую сказку и сгенерируй 3 вопроса для размышлений.{age_info}

СКАЗКА:
{story_text}

Сгенерируй 3 вопроса, которые помогут ребенку:
- Понять мораль сказки
- Задуматься о своих поступках
- Применить уроки сказки к своей жизни
...
3. ПРО РЕФЛЕКСИЮ И САМОПОЗНАНИЕ - Что это говорит о характере героя?
4. ПРО ЭТИКУ И МОРАЛЬ - Где граница между правильным и неправильным?
...
- Что случилось, когда герой не поделился?"""
    ...
    def _get_default_questions(self, age_group: str) -> List[str]:
        ...
        defaults = {
            ...
            "13+": [
                "Какие моральные принципы отражены в этой сказке?",
