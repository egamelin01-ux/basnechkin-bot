# Сказочник - Telegram-бот для генерации сказок

Telegram-бот, который помогает родителям наставлять детей через художественные сказки по мотивам сказок Крылова И.А., в которых ребёнок становится персонажем истории.
...
- **DeepSeek API** - генерация текста сказок
- **Google Sheets API** - база данных профилей и последних 5 сказок на пользователя
...
2. Назовите её, например, "Сказочник БД"
...
**Лист 2: Stories** (сказки, максимум 5 на пользователя)
...
3. Ответьте на вопросы анкеты:
   ...
   - Возраст
   - Черты характера
4. После завершения анкеты бот автоматически сгенерирует первую сказку-знакомство
5. В дальнейшем просто отправляйте сообщения боту, и он будет генерировать новые сказки
...
- **Хранение последних 5 сказок**: в Google Sheets хранится только последние 5 сказок на пользователя (старые автоматически удаляются)
- **Кэширование профилей**: профили кэшируются на 5 минут для оптимизации
- **Разбиение длинных сообщений**: сказки автоматически разбиваются на части, если превышают лимит Telegram
